// 安全标志图集管理系统 - 完整版（带组合配置）
const API_BASE = '/api';
let selectedImageFile = null;
let allSigns = [];
let allScenes = [];
let currentSceneId = null;
let selectedSigns = new Set(); // 存储已选标志的ID
...
function showPage(pageId) {
    ...
    switch(pageId) {
        ...
        case 'create-scene': resetCreateSceneForm(); break;
        case 'scene-list': loadScenes(); break;
        case 'scene-detail': break; // 需要sceneId参数
        case 'configure-signs': break; // 需要sceneId参数
    }
}
            ...
            html += `
                <div class="sign-card" style="margin-bottom: 15px;">
                    ...
                    <div style="margin-top: 15px; display: flex; gap: 10px;">
                        <button class="btn" onclick="viewSceneDetail(${scene.id})" style="padding: 5px 15px; font-size: 0.9rem;">查看详情</button>
                        <button class="btn btn-success" onclick="configureSceneSigns(${scene.id})" style="padding: 5px 15px; font-size: 0.9rem;">配置标志</button>
                    </div>
                </div>
            `;
        });
...
async function viewSceneDetail(sceneId) {
    try {
        ...
        // 显示场景详情页面
        document.getElementById('scene-detail-title').innerHTML = `场景详情 - ${scene.scene_code}`;
        document.getElementById('scene-detail-code').innerHTML = scene.scene_code;
        document.getElementById('scene-detail-name').innerHTML = scene.scene_name;
        document.getElementById('scene-detail-dept').innerHTML = scene.department;
        document.getElementById('scene-detail-risk').innerHTML = getRiskLevelText(scene.risk_level);
        document.getElementById('scene-detail-location').innerHTML = scene.location_description || '暂无';
        document.getElementById('scene-detail-notes').innerHTML = scene.installation_notes || '暂无';
        document.getElementById('scene-detail-created').innerHTML = formatDate(scene.created_at);
        
        // 加载场景的标志
        loadSceneSigns(sceneId);
        
        // 切换到详情页面
        showPage('scene-detail');
...
// 加载场景的标志
async function loadSceneSigns(sceneId) {
    const container = document.getElementById('scene-signs-container');
    container.innerHTML = '<div class="loading">加载场景标志...</div>';
    ...
    try {
        ...
        if (!scene.signs || scene.signs.length === 0) {
            container.innerHTML = '<div class="message">该场景尚未配置任何标志</div>';
            return;
        }
        
        let html = '<div class="sign-grid">';
